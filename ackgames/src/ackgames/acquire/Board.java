...
public class Board extends Canvas
{
    ...
    synchronized public boolean mouseDown(Event e,int x, int y)
    {
        ...
        if (e.modifiers == Event.SHIFT_MASK){
	        ...
	        } else if (m_gameBoard.getGameState() == GameBoard.PLACETILE){
				Tile ti= m_gameBoard.getM_tile()[y/(h/9)][x/(w/12)];
				if(ti.getState() == Tile.EMPTY && ti.isM_inRack()){
					Client.A.playTile(ti);
				}
			}
	    }
	    return true;
