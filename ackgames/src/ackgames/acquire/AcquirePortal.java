...
public class AcquirePortal extends HttpServlet {
    ...
    public String getJsonStr(HttpSession session ,int state, StringBuffer tile, StringBuffer msg,StringBuffer arg,StringBuffer chat)
    {
        return Json.createObjectBuilder()
                .add("state", state)
                .add("board",bg.getState((String)(session.getAttribute("gameId"))))
                .add("tile", tile.toString())
                .add("stats",bg.getStats((String)(session.getAttribute("gameId"))))
                .add("msg", msg.toString())
                .add("arg", arg.toString())
                .add("chat", chat.toString())
                .build().toString();
    }
...
//                result+="arg:"+outArg+"~";
//                result+="chat|"+chat;
                res = getJsonStr(session , s, tile, msg, outArg,chat);
                break;
            case 3:
